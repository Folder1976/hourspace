<?php

//Menu
$_['text_a_place_where'] = 'место, где';
$_['text_everything_is_free_except_time'] = 'все бесплатно, кроме времени';
$_['text_rates'] = 'Цены';
$_['text_nik'] = 'NIS';
$_['text_per_hour'] = 'В ЧАС';
$_['text_discounst_and_membership'] = 'у нас есть скидки и абонементы';
$_['text_details'] = 'Подробнее';
$_['text_hire'] = 'Аренда';
$_['text_we_are_located_in'] = 'Мы занимаем уютное трехэтажное помещение в самом центре Тель-Авива.
Интерьер был создан для вас нашим коллективом с особым вниманием к деталям, 
чтобы вы чувствовали себя комфортно, как дома.';
$_['text_time_club'] = '- Антикафе -';
$_['text_coworcing'] = '- Коворкинг -';
$_['text_creative_space'] = '- Творческая среда -';
$_['text_co_working_mode'] = 'до 6 вечера мы вместе<br>работаем и учимся<br>в спокойной, приятной<br>атмосфере';
$_['text_child_mode'] = 'по вечерам мы вместе<br>отдыхаем, принимаем<br>участие в различных<br>мероприятиях';
$_['text_join_us_on'] = '<a href="//www.facebook.com/hourspaceTLV" target="_blank">Подписывайтесь! <img src="/catalog/view/theme/default/image/fb.png" alt="fb"><br>Здесь вы найдете актуальное расписание наших мероприятий.</a>';
$_['text_whichever'] = 'какой бы вариант пребывания у нас вы не выбрали,<br>в вашем полном распоряжении';
$_['text_tea'] = 'чай';
$_['text_snacks'] = 'печенье';
$_['text_coffee'] = 'кофе';
$_['text_lemonade'] = 'лимонад';
$_['text_printer'] = 'принтер';
$_['text_chargers'] = 'зарядки';
$_['text_scanner'] = 'сканер';
$_['text_wifi'] = 'wifi';
$_['text_board_games'] = 'настольные игры';
$_['text_books'] = 'книги';
$_['text_projector'] = 'проектор';
$_['text_playstation'] = 'playstation';
